...
- Runs within existing Groq free-tier pacing; cached so a bottle enriches at most once (until explicitly refreshed) — ✅ enrich-once gated by `flavorEnrichedAt`; `refresh` overrides
...
**SP:** 5 — **server engine built in 3 passes:** (1) canonical taxonomy + matcher + model, (2) on-demand `enrichBottleFlavor` callable (enrich-once, free-tier), (3) news-feed flavor seed. The client trigger + pre-fill is **BB-186**.
