...
### BB-181 — Structured Flavor Profile & Curated Flavor Wheel
**As a** user, **I want** a fuller, well-organized flavor vocabulary with the common notes up front and the rest a tap away, **so that** I can capture nose/palate/finish precisely without scrolling a wall of chips.

**Context:** the structured nose/palate/finish picker already exists
(`FlavorTagSelectorComponent` + `FLAVOR_TAG_GROUPS`, ~40 tags). This story
expands that seed into a curated whiskey flavor wheel and adds common/extended
tiers. The resulting **canonical vocabulary is the backbone** the AI maps to
(BB-185) and auto-populate consumes (BB-186) — which is what keeps flavor data
controlled and legally safe (no verbatim third-party notes ever stored).

**AC:**
- `FLAVOR_TAG_GROUPS` expands into a curated whiskey flavor wheel (established categories: Sweet, Orchard/Tropical/Dried Fruit, Citrus, Baking & Savory Spice, Oak/Wood, Grain/Cereal, Floral, Nutty, Herbal/Vegetal, Smoke/Peat, Dairy/Creamy, Earthy/Mineral, and a small Funk/Off group)
- Each category splits into a **common** tier (shown by default) and an **extended** tier (revealed by a per-stage "Show more" toggle)
- The picker keeps the existing nose / palate / finish stages, multi-select chips, and finish length; free-text notes remain
- Tags stay a **controlled vocabulary** (canonical labels/slugs), not free strings — this single list is what the AI (BB-185) and auto-populate (BB-186) reference
- Selected tags render on entry detail as today
- The taxonomy (categories + tiers) is documented so the AI prompt/matcher stays in sync

**SP:** 3 *(picker/storage already exist; scope is the taxonomy + tiering)*
...
## Epic 16: Guided Tasting & Catalog Intelligence *(Post-Social)*

> Depends on the **BB-181** canonical taxonomy. Replaces the original
> "web-scrape pro reviews" idea with AI enrichment constrained to our own
> vocabulary — a legal safeguard (no third-party prose stored) and a
> data-consistency win (every note is a controlled tag).

### BB-185 — AI Flavor Enrichment (mapped to the taxonomy) ✅ *(server code-complete)*
**As a** user, **I want** catalog bottles to carry a flavor profile drawn from reliable signals, **so that** logging can suggest notes I might not pick out myself.

**AC:**
- A bottle's flavor profile (nose/palate/finish tags from the **BB-181 vocabulary**) is generated by the Groq LLM and cached on the `/bourbons` doc — ✅ `FlavorProfile` cached on `/bourbons`
- Two feeds: (a) the existing news extraction (BB-130) also pulls flavor cues and maps them to canonical tags; (b) on-demand generation from name/distillery/category when no article exists — ✅ (a) `seedArticleFlavor` in the news pipeline; (b) `enrichBottleFlavor` callable
- The model is prompted with the canonical tag list and **constrained to it**; any free text it returns is fuzzy-matched to the nearest canonical tag and **dropped if no confident match** — never stored verbatim — ✅ `matchCanonicalTag` (exact → phrase-contains → Dice ≥ 0.6, else drop)
- Runs within existing Groq free-tier pacing; cached so a bottle enriches at most once (until explicitly refreshed) — ✅ gated by **adequacy** (≥5 tags across ≥2 stages), not mere existence: an adequate profile makes no model call; a thin one is upgraded. Feeds **merge/union** (capped 6/stage) so multiple articles + feed (b) accumulate a consensus profile rather than first-writer-wins. A rate-paced **`sweepFlavorEnrichment`** (hourly, + manual `backfillFlavorEnrichment`) proactively brings inadequate bottles up to standard, with a 14-day retry cooldown
- A source/confidence marker distinguishes AI-suggested from user-confirmed notes — ✅ `source: 'ai'` on the catalog profile; user-confirmed tags live on the log entry
- **No third-party tasting text is ever persisted** — ✅ only canonical labels are ever stored

**SP:** 5 — **server engine built in 4 passes:** (1) canonical taxonomy + matcher + model, (2) on-demand `enrichBottleFlavor` callable, (3) news-feed flavor seed, (4) **quality hardening** — adequacy gate + source merge + proactive backfill sweep (so thin article seeds never lock in weak profiles while the DB is seeding). The client trigger + pre-fill is **BB-186**.
...
source — **OpenStreetMap Overpass** (purpose-built for category-POI-near-a-point),
called **server-side from a Cloud Function** to cache results and respect Overpass
...
**As a** user, **I want** a bottle's suggested notes to reflect what the community actually tasted, **so that** suggestions improve over time.

**Context:** blends users' own **confirmed** flavor tags (BB-181/186) per bottle
into an aggregate, complementing the AI seed (BB-185). Needs a real user base to
be meaningful, so it sits behind the single-user/circle phase.

**AC (draft):**
- Aggregate confirmed flavor tags across users per `bourbonId` into a ranked suggestion set
- Blend/weight against the AI-seeded profile; aggregate-only, never attributed to individuals
- Feeds the same auto-populate path (BB-186)

**SP:** TBD *(backlog)*

---

### BB-189 — Nearby Venue Picker (Bars / Restaurants)
**As a** user, **I want** to also pick nearby bars/restaurants when logging, **so that** on-premise finds are as fast to log as retail ones.

**Context:** the second POI category for BB-187, split per the Untappd-style
Retailer/Venue separation. Same Overpass + Cloud Function mechanism, different OSM
tags (`amenity=bar/pub/restaurant`).

**AC (draft):**
- Reuses the BB-187 picker infrastructure with a Venue category
- Queries `amenity=bar/pub/restaurant` near the user's coordinates
- Tapping auto-fills the venue name + city/state
