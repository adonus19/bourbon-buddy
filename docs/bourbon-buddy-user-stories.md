...
### BB-185 — AI Flavor Enrichment (mapped to the taxonomy)
...
**AC:**
- A bottle's flavor profile (nose/palate/finish tags from the **BB-181 vocabulary**) is generated by the Groq LLM and cached on the `/bourbons` doc
- Two feeds: (a) the existing news extraction (BB-130) also pulls flavor cues and maps them to canonical tags; (b) on-demand generation from name/distillery/category when no article exists
- The model is prompted with the canonical tag list and **constrained to it**; any free text it returns is fuzzy-matched to the nearest canonical tag and **dropped if no confident match** — never stored verbatim
- Runs within existing Groq free-tier pacing; cached so a bottle enriches at most once (until explicitly refreshed)
- A source/confidence marker distinguishes AI-suggested from user-confirmed notes
- **No third-party tasting text is ever persisted**
